...
const FormFieldList = ({ fields, sensors, onDragEnd, onFieldUpdate, onFieldRemove, formType }) => (
        ...
        <DndContext  modifiers={[restrictToParentElement]} sensors={sensors} collisionDetection={closestCorners} onDragEnd={onDragEnd}>
            <FormTypeContext.Provider value={formType}>
                <SortableContext items={fields.map((field) => field.id)}>
                    {fields.map((field) => (
                        <SortableItem key={field.id} field={field} onFieldUpdate={onFieldUpdate} onFieldRemove={onFieldRemove} />
                    ))}
                </SortableContext>
