...
export const checkSubmissionLimit = async (req, res, next) => {
    try {
        ...
        // Get current month's submission count for this user
        const now = new Date();
        ...
        if (currentSubmissionCount >= limits.submissionLimit) {
            return res.status(403).json({
                success: false,
                error: `Monthly submission limit exceeded. Your ${planName} plan allows ${limits.submissionLimit} submissions per month. You have used ${currentSubmissionCount} submissions this month.`,
                limit: {
        ...
        // Add plan and usage info to request
        req.userPlan = { limits, planName };
        req.submissionUsage = {
            current: currentSubmissionCount,
